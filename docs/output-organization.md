...
看板会用这些结构化列直接展示套磁判断、核心匹配、教师主页方向和风险，不会重新评分。套磁日历与表格共用 `contact_status.json` 和筛选条件，桌面显示月历、移动端显示议程，并提供缺日期列表和可关闭的密集发送提醒。日历事件以颜色和文字表达 `回复情况`，多回复记录以最高进展决定主色但保留完整回复链。首次选择“已套磁”且 `套磁时间` 为空时，会按本机日期补入当天；已有日期保持不变。旧工作簿缺列时页面会标记“旧数据”；要获得完整的锚点和分来源得分展示，需要按当前 policy 重跑三阶段。
