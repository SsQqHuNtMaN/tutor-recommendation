...
下文以 `<profile_root>` 统一表示 `outputs/`（旧画像）或
`outputs/by_profile/<profile_id>/`（命名画像）。
...
```text
<profile_root>/<school_slug>/<college_slug>/
...
```text
<profile_root>/<school_slug>/<college_slug>/<school_slug>_<college_slug>_teacher_match_full_research.xlsx
```
...
目标键在 `src/tutor_recommendation/teacher_match_targets.py` 中定义。统一入口会同时配置学校、学院、画像目录和论文证据流程；不要绕过目标键手工拼接输出路径。

源码集中在 `src/tutor_recommendation/`；根目录只保留 `tutor.py` 引导入口，兼容包装器位于 `scripts/legacy/`。个人材料和画像放在 `user_private/`，旧 `data/private/` 继续兼容，公开模板放在 `data/templates/`。正式研究产物、cache 和人工联系状态全部跟随所选 `<profile_root>`。
...
服务会读取所选画像的 `*_teacher_match_full_research.xlsx`，并把套磁编辑写入同一 `<profile_root>/contact_status.json`。画像可在页头切换，每个请求显式携带画像 ID。需要把 JSON 状态写回 Excel 时运行：
...
- WebSearch cache：`<profile_root>/<school_slug>/<college_slug>/web_search_cache/`
...
后续论文证据、arXiv、网页和看板阶段应以最新工作簿行作为底稿，原样保留这些补充列。
...
最终补全阶段以 target 对应的第二阶段证据表（DBLP 或数学文献）为底，再叠加 checkpoint 中的 arXiv/web 证据。checkpoint 指纹包含教师 ID、画像 ID 与哈希、evidence profile、论文窗口、policy/schema 版本和关键输入哈希；任何相关变化都会使旧记录失效。`--finalize-only` 默认要求 100% 有效覆盖，可先运行 `tutor doctor <target>` 检查；`--allow-partial` 只用于明确接受缺失深检索证据的场景。
...
- 数学文献表：`<school_slug>_<college_slug>_teacher_match_publications.xlsx`
...
1. 为学校和学院确定 slug。
2. 在 `src/tutor_recommendation/teacher_match_targets.py` 新增目标配置，包括 evidence profile 和论文窗口。
3. 在 `src/tutor_recommendation/collectors/` 实现或复用解析器，并在 `collectors/registry.py` 显式绑定。
4. 运行第一阶段，确认输出生成在所选画像的学校/学院目录。
...
```powershell
Remove-Item -LiteralPath '<profile_root>\<school_slug>\<college_slug>\dblp_cache' -Recurse -Force
Remove-Item -LiteralPath '<profile_root>\<school_slug>\<college_slug>\math_publication_cache' -Recurse -Force
Remove-Item -LiteralPath '<profile_root>\<school_slug>\<college_slug>\arxiv_cache' -Recurse -Force
Remove-Item -LiteralPath '<profile_root>\<school_slug>\<college_slug>\web_cache' -Recurse -Force
```
