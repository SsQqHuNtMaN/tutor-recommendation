...
“套磁日历”复用当前学校、学院和其他筛选。桌面显示七列月历，移动端显示按日期排列的议程；可用上月、今天、下月导航。日历事件颜色和文字表示 `回复情况`：已发、官回、添加微信、约面试、考核、已满或未记录。多个回复同时存在时，以最高进展决定主色，紧凑月历显示最高进展和数量，移动议程保留完整回复链。日历还会列出缺少 `套磁时间` 的已套磁记录，并对同日同学院集中发送、七天内同校密集发送给出可关闭的非阻断提醒。点击教师条目打开现有详情；修改日期或回复情况后日历立即更新并写入同一份 `contact_status.json`。

后续已确认的日历、列表和可折叠详情栏整合方案见 [Viewer 布局 TODO](viewer-integrated-layout-todo.md)。
